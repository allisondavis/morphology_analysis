...
plot6.1<- autoplot(PCA, data = raw2, colour='SPP', loadings=FALSE, loadings.label=FALSE, frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal()
plot6.1
...
sd.lat.D1 <- rstandard(reg.lat.D1)
...
sd.lat.P11 <- rstandard(reg.lat.P11)
...
sd.lat.P2.L1 <- rstandard(reg.lat.P2.L1)
...
sd.lat.P2.R1 <- rstandard(reg.lat.P2.R1)
...
sd.lat.A1 <- rstandard(reg.lat.A1)
...
sd.lat.P1.R1 <- rstandard(reg.lat.P1.R1)
...
sd.lat.LLSC1 <- rstandard(reg.lat.LLSC1)
...
sd.lat.SALL1 <- rstandard(reg.lat.SALL1)
...
sd.lat.SBLL1 <- rstandard(reg.lat.SBLL1)
...
sd.lat.SBDF1 <- rstandard(reg.lat.SBDF1)
...
sd.lat.BD1 <- rstandard(reg.lat.BD1)
...
sd.lat.CPD1 <- rstandard(reg.lat.CPD1)
...
sd.lat.CPL1 <- rstandard(reg.lat.CPL1)
...
sd.lat.PreDL1 <- rstandard(reg.lat.PreDL1)
...
sd.lat.DbL1 <- rstandard(reg.lat.DbL1)
...
sd.lat.HL1 <- rstandard(reg.lat.HL1)
...
sd.lat.HD1 <- rstandard(reg.lat.HD1)
...
sd.lat.HW1 <- rstandard(reg.lat.HW1)
...
sd.lat.SnL1 <- rstandard(reg.lat.SnL1)
...
sd.lat.OL1 <- rstandard(reg.lat.OL1)
...
sd.form.D1 <- rstandard(reg.form.D1)
...
sd.form.P11 <- rstandard(reg.form.P11)
...
sd.form.P2.L1 <- rstandard(reg.form.P2.L1)
...
sd.form.P2.R1 <- rstandard(reg.form.P2.R1)
...
sd.form.A1 <- rstandard(reg.form.A1)
...
sd.form.P1.R1 <- rstandard(reg.form.P1.R1)
...
sd.form.LLSC1 <- rstandard(reg.form.LLSC1)
...
sd.form.SALL1 <- rstandard(reg.form.SALL1)
...
sd.form.SBLL1 <- rstandard(reg.form.SBLL1)
...
sd.form.SBDF1 <- rstandard(reg.form.SBDF1)
...
sd.form.BD1 <- rstandard(reg.form.BD1)
...
sd.form.CPD1 <- rstandard(reg.form.CPD1)
...
sd.form.CPL1 <- rstandard(reg.form.CPL1)
...
sd.form.PreDL1 <- rstandard(reg.form.PreDL1)
...
sd.form.DbL1 <- rstandard(reg.form.DbL1)
...
sd.form.HL1 <- rstandard(reg.form.HL1)
...
sd.form.HD1 <- rstandard(reg.form.HD1)
...
sd.form.HW1 <- rstandard(reg.form.HW1)
...
sd.form.SnL1 <- rstandard(reg.form.SnL1)
...
sd.form.OL1 <- rstandard(reg.form.OL1)
...
### NONPARAMETRIC TESTS--DID THIS BECAUSE SHIT WASN'T NORMAL BUT MIKE SAID JUST TO TRANSFORM.



## Levene's tests {.tabset .tabset-fade .tabset-pills}
...
## Mann Whitney U tests {.tabset .tabset-fade .tabset-pills}
...
(MW_BD <- wilcox.test(abs.res.BD~SPP, data=raw3, conf.int=T))

(MW_CPD <- wilcox.test(abs.res.CPD~SPP, data=raw3, conf.int=T))

(MW_CPL <- wilcox.test(abs.res.CPL~SPP, data=raw3, conf.int=T))

(MW_PreDL <- wilcox.test(abs.res.PreDL~SPP, data=raw3, conf.int=T))

(MW_DbL <- wilcox.test(abs.res.DbL~SPP, data=raw3, conf.int=T))

(MW_HL <- wilcox.test(abs.res.HL~SPP, data=raw3, conf.int=T))

(MW_HD <- wilcox.test(abs.res.HD~SPP, data=raw3, conf.int=T))

(MW_HW <- wilcox.test(abs.res.HW~SPP, data=raw3, conf.int=T))

(MW_SnL <- wilcox.test(abs.res.SnL~SPP, data=raw3, conf.int=T))

(MW_OL <- wilcox.test(abs.res.OL~SPP, data=raw3, conf.int=T))
...
raw5 <- cbind(raw4, sd.res.D, sd.res.P1, sd.res.P1.R, sd.res.LLSC, sd.res.SBLL, sd.res.BD, sd.res.CPD, sd.res.CPL, sd.res.PreDL, sd.res.DbL, sd.res.HL, sd.res.HD, sd.res.HW, sd.res.SnL, sd.res.OL)
...
raw5a <- cbind(raw5, raw5a)

raw5b <- cbind(raw5a[1:32], stack(raw5a[49:78])) 
raw5b$abs_sd = abs(raw5b$values)

lat.raw5 <- raw5b[raw5b$SPP == "p.latipinna",]

form.raw5 <- raw5b[raw5b$SPP == "p.formosa",]

A2.lat <- aov(abs_sd~QUARTILE, data=lat.raw5)
summary(A2.lat)

A2.form <- aov(abs_sd~QUARTILE, data=form.raw5)
summary(A2.form)

#between species

A2 <- aov(abs_sd~QUARTILE*SPP, data=raw5b)
summary(A2)




#BELOW: I compiled all the residual scores into each quartile (so all residuals for all traits in Q1, ex) and then ran the anovas below. BUT the residuals are not unitless--so I need to standardize them. Will try this again above with standardized residuals
library(curl)

df <- curl("https://raw.githubusercontent.com/allisondavis/morphology_analysis/master/ANOVA_res-data.csv")

df <- read.csv(df, header = TRUE, sep = ",", stringsAsFactors = TRUE)

head(df)

lat.df <- df[df$Spp == "lat",]
head(lat.df)

form.df <- df[df$Spp == "form",]
head(form.df)

#within species

A.lat <- aov(X.U.FEFF.Residuals~Location, data=lat.df)
summary(A.lat)

A.form <- aov(X.U.FEFF.Residuals~Location, data=form.df)
summary(A.form)

#between species

A <- aov(X.U.FEFF.Residuals~Location*Spp, data=df)
summary(A)
...
## Old Tests {.tabset .tabset-fade .tabset-pills} 
...
Old tests: turns out my data is not normal, so I can't use the F-tests or T-tests. Keeping it here for record keeping.
...
ttest.abs.SALL <- t.test(abs.lat.SALL, abs.form.SALL, alternative = "two.sided")
ttest.abs.SALL
