...
```{r}
avg_comp <- matrix(c(mean(lat$D), mean(form$D), mean(lat$P1), mean(form$P1), mean(lat$P2.L), mean(form$P2.L), mean(lat$P2.R), mean(form$P2.R), mean(lat$A), mean(form$A), mean(lat$A), mean(form$A), mean(lat$P1.R), mean(form$P1.R), mean(lat$LLSC), mean(form$LLSC), mean(lat$SALL), mean(form$SALL), mean(lat$SBLL), mean(form$SBLL), mean(lat$SBDF), mean(form$SBDF), mean(lat$TL), mean(form$TL), mean(lat$SL), mean(form$SL), mean(lat$BD), mean(form$BD), mean(lat$CPD), mean(form$CPD), mean(lat$CPL), mean(form$CPL), mean(lat$PreDL), mean(form$PreDL), mean(lat$DbL), mean(form$DbL), mean(lat$HL), mean(form$HL), mean(lat$HW), mean(form$HW), mean(lat$HD), mean(form$HD), mean(lat$SnL), mean(form$SnL), mean(lat$OL), mean(form$OL), ))
...
**ALL BUT P2.R, P2.L, A, SALL, AND SBDF WERE SIGNIFICANT FOR ONE OR BOTH SPECIES**
...
raw3 <- cbind(raw1, abs.res.D, abs.res.P1, abs.res.P1.R, abs.res.LLSC, abs.res.SBLL, abs.res.BD, abs.res.CPD, abs.res.CPL, abs.res.PreDL, abs.res.DbL, abs.res.HL, abs.res.HD, abs.res.HW, abs.res.SnL, abs.res.OL)
...
var.anal <- var.test(lat.1$A, form.1$A, alternative = "two.sided")
var.anal

var.sca.abv <- var.test(lat.1$SALL, form.1$SALL, alternative = "two.sided")
var.sca.abv
...
average.table <- matrix(c(ttest.abs.D1$p.value, ttest.abs.P11$p.value, var.rig.pel$p.value, var.left.pel$p.value, var.anal$p.value, ttest.abs.P1.R1$p.value, ttest.abs.LLSC1$p.value, var.sca.abv$p.value, ttest.abs.SBLL1$p.value, var.sca.df$p.value, ttest.abs.BD1$p.value, ttest.abs.CPD1$p.value, ttest.abs.CPL1$p.value, ttest.abs.PreDL1$p.value, ttest.abs.DbL1$p.value, ttest.abs.HL1$p.value, ttest.abs.HD1$p.value, ttest.abs.HW1$p.value, ttest.abs.SnL1$p.value, ttest.abs.OL1$p.value), ncol=1, byrow=TRUE)

colnames(average.table)<- c('p-value for variance')
