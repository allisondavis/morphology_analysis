...
dig.meas %>%
  ...
  tab_style(
    style = cell_borders(sides = "right", color = "lightgray", weight = px(1), style = "dashed"),
    locations = cells_body(columns = c(First.landmark, Second.landmark))) %>%
  cols_align(
    align = "center",
    columns = everything())
...
ba_stats_all %>%
  select(characteristic, t_value, p_value, bias_CI, LOA_range) %>%
  gt() %>%
  ...
  fmt_number(columns = c(t_value, p_value), decimals = 3) %>%
  fmt(columns = c("p_value"), fns = function(x) {  
    ifelse(x < 0.001, "p < 0.001", formatC(x, format = "f", digits = 3))}) %>%
  tab_style(
    style = cell_borders(sides = "right", color = "lightgray", weight = px(1.5)), 
    locations = cells_body(columns = c(characteristic, t_value, p_value, bias_CI))) %>%
  cols_align(
    align = "center",
    columns = everything())
...
pc1 <- t.test(zw.dig$Dim.1, zw.man$Dim.1, paired= TRUE, alternative = "two.sided") # Not signficant (p=1)

pc2 <- t.test(zw.dig$Dim.2, zw.man$Dim.2, paired= TRUE, alternative = "two.sided") # Not significant (p=1)
...
ttest_result <- tibble(
  Dimension = c("PC1", "PC2"),
  t_value = c(pc1$statistic[1], pc2$statistic[1]),
  p_value = c(pc1$p.value[1], pc2$p.value[1])
) %>%
  mutate(Significance = case_when(
    p_value < 0.001 ~ "***",
    p_value < 0.01 ~ "**",
...
ttest_result %>%
  ...
  tab_style(
    style = cell_borders(sides = "right", color = "lightgray", weight = px(1.5)),
    locations = cells_body(columns = c(Dimension, t_value, p_value, Significance)))
...
pc1V <- leveneTest(Dim.1~Method, data=z.wide) # Not significant (p=0.96)
pc2V <- leveneTest(Dim.2~Method, data=z.wide) # Not significant (p=0.18)

# Will display results in a table

levenes_result <- tibble(
  Dimension = c("PC1", "PC2"),
...
levenes_result %>%
  ...
  tab_style(
    style = cell_borders(sides = "right", color = "lightgray", weight = px(1.5)),
    locations = cells_body(columns = c(Dimension, F_value, p_value, Significance)))
