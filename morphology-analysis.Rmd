...
In this analysis, I will compare the principle components after centering and scaling the data. A PCA analysis will help us determine what aspects of morphology influence the variation in our data the most without worrying about differences in scales/measurements. Currently, data consists of **28 Sailfin and 45 Amazon**. 

***Will use the original raw dataset rather than any outlier-removed datasets.***
...
PCA1 <- prcomp(raw1[, 18:29], center=TRUE, scale. = TRUE)
summary(PCA1)
...
library(AMR)
library(ggplot2)
raw.spp <- c(rep("Sailfin", 27), rep("Amazon", 44))

plot1<- ggplot_pca(PCA1) + ggtitle("Plot 1") + theme_minimal()
plot1

plot2 <- ggplot_pca(PCA1, ellipse = TRUE, color= raw.spp, groups = raw.spp) + ggtitle("Plot 2") + theme_minimal()
plot2
...
plot4<- autoplot(PCA1, data = raw1, colour='SPP', loadings=TRUE, loadings.colour='navyblue', loadings.label=TRUE, loadings.label.colour='navyblue', loadings.label.size=5, loadings.label.vjust= 1, loadings.label.hjust= 1.2, frame=TRUE, frame.type='norm')+ ggtitle("Plot 4") + theme_minimal()
plot4

```

### Preliminary data inspection {.tabset .tabset-fade .tabset-pills}
...
ggplot(raw2, aes(x=SPP, y=P1.R)) + geom_boxplot(fill="transparent") + xlab("Species") + ylab("Ray count") + ggtitle("Right pectorcal ray comparison") + scale_y_continuous(limits = c(12, 15)) + geom_dotplot(binaxis = "y", stackdir = "center", method="histodot",binwidth = 1,  dotsize = 0.08, fill= "black") + theme_classic() + theme(text = element_text(size = 16)) + theme(legend.position = "none") + theme(panel.background = element_rect(fill = "transparent"), plot.background = element_rect(fill = "transparent", color = NA))
...
*not sure if I should do anymore??* There appear to be some outliers; for example, two points are well below the rest for pectoral count, and upon inspection at least one of these points has the note that the pectoral looks ripped. Will do an outlier test to remove what I can to ensure these outliers are not affecting the significance.
...
####  T.TESTS (outlier removed) {.tabset .tabset-fade .tabset-pills}
...
####  VARIANCE TESTS (outier removed) {.tabset .tabset-fade .tabset-pills}
...
### Corrections for body size {.tabset .tabset-fade .tabset-pills}

The amazon mollies are on averages larger than the sailfin mollies, as shown in the t-test above. To ensure body size is not affecting our comparison of these characteristics, I will correct for body size in two main ways: 1) I will run an ANCOVA for each characteristic using standard length (SL) as a covariate. Each model will resemble: trait \~ species \* size. This will tell us if the mean differences of the traits are driven by the species or by the body length. 2) To correct for body size with variance, I will regress each trait against SL and use the residuals in the F-test. The original residuals should express both negative and positive values. Additionally, I will run F-tests with the absolute value of the residuals and note any differences in levels of significance (i.e. if it was sig in one but not the other). *need to figure out why doing this was logical again.*

#### ANCOVA {.tabset .tabset-fade .tabset-pills}
...
-   the only interactions that come out significant are PreDL and DbL
  ...
  * <font color = "darkorchid4"> ***Unable to perform ANCOVA using the method from <https://www.datanovia.com/en/lessons/ancova-in-r/>. Not sure what's going on with the data.***
