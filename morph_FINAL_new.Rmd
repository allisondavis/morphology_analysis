...
# Initial steps {.tabset .tabset-fade .tabset-pills}
...
### SW Test {.tabset .tabset-fade .tabset-pills}
```{r}
...
### QQ plots {.tabset .tabset-fade .tabset-pills}
```{r}
...
# Comparing variation (Levene and KW tests) {.tabset .tabset-fade .tabset-pills}
...
general formula: leveneTest(dependent~independent, dataframe)
  For me, I am interested in if the variance of a trait (dependent) differs between two species (independent), so it will be leveneTest(trait~spp, df).
...
average.table <- matrix(c(MW_D$p.value, mean(abs.lat.D), mean(abs.form.D), MW_P1$p.value, mean(abs.lat.P1), mean(abs.form.P1), LT_P2R$p.value, mean(lat$P2.R), mean(form$P2.R), LT_P2L$p.value, mean(lat$P2.L), mean(form$P2.L), LT_A$p.value, mean(lat$A), mean(form$A), MW_P1R$p.value, mean(abs.lat.P1.R), mean(abs.form.P1.R), MW_LLSC$p.value, mean(abs.lat.LLSC), mean(abs.form.LLSC), LT_SALL$p.value, mean(lat$SALL), mean(form$SALL), LT_SBLL$p.value, mean(lat$SBLL), mean(form$SBLL), LT_SBDF$p.value, mean(lat$SBDF), mean(form$SBDF), MW_BD$p.value, mean(abs.lat.BD), mean(abs.form.BD), MW_CPD$p.value, mean(abs.lat.CPD), mean(abs.form.CPD), MW_CPL$p.value, mean(abs.lat.CPL), mean(abs.form.CPL), MW_PreDL$p.value, mean(abs.lat.PreDL), mean(abs.form.PreDL), MW_DbL$p.value, mean(abs.lat.DbL), mean(abs.form.DbL), MW_HL$p.value, mean(abs.lat.HL), mean(abs.form.HL), MW_HD$p.value, mean(abs.lat.HD), mean(abs.form.HD), MW_HW$p.value, mean(abs.lat.HW), mean(abs.form.HW), MW_SnL$p.value, mean(abs.lat.SnL), mean(abs.form.SnL), MW_OL$p.value, mean(abs.lat.OL), mean(abs.form.OL), LT_FLA$p.value, mean(lat$FLA), mean(form$FLA)), ncol=3, byrow=TRUE)
...
plot_multi_histogram(raw3, "abs.res.SALL", "SPP")
...
## Zones {.tabset .tabset-fade .tabset-pills}
