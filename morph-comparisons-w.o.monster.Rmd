---
title: "morph_comparisons-w.o.monster"
...
```{r}
raw1b[paste0(names(raw1b), '_log')] <- log(raw1b[, 26:35], 10)
...
shapiro.test(raw1b$SL_log)

shapiro.test(raw1b$BD_log)

shapiro.test(raw1b$CPD_log)

shapiro.test(raw1b$CPL_log)

shapiro.test(raw1b$PreDL_log)

shapiro.test(raw1b$DbL_log)

shapiro.test(raw1b$HL_log)

shapiro.test(raw1b$HD_log)

shapiro.test(raw1b$HW_log)

shapiro.test(raw1b$SnL_log)

shapiro.test(raw1b$OL_log)
...
qqnorm(raw1b$CPD_log)
qqline(raw1b$CPD_log)

qqnorm(raw1b$CPL_log)
qqline(raw1b$CPL_log)

qqnorm(raw1b$PreDL_log)
qqline(raw1b$PreDL_log)

qqnorm(raw1b$DbL_log)
qqline(raw1b$DbL_log)
...
qqnorm(raw1b$SnL_log)
qqline(raw1b$SnL_log)
...
```{r}
#raw1b$sqR_BD ='^'(raw1b$BD,1/2)
#raw1b$sqR_PreDL ='^'(raw1b$PreDL,1/2)
#raw1b$sqR_DbL ='^'(raw1b$DbL,1/2)

#shapiro.test(raw1b$sqR_BD) #didn't work
#shapiro.test(raw1b$sqR_PreDL)#worked
#shapiro.test(raw1b$sqR_DbL)#worked

#qqnorm(raw1b$sqR_BD)
#qqline(raw1b$sqR_BD)

#qqnorm(raw1b$sqR_PreDL)
#qqline(raw1b$sqR_PreDL)

#qqnorm(raw1b$sqR_DbL)
#qqline(raw1b$sqR_PreDL)

#cubed root
raw1b$cbR_BD ='^'(raw1b$BD,1/3)
raw1b$cbR_PreDL ='^'(raw1b$PreDL,1/3)
raw1b$cbR_DbL ='^'(raw1b$DbL,1/3)

shapiro.test(raw1b$cbR_BD) #didn't work, but closest to normal
shapiro.test(raw1b$cbR_PreDL)#worked
shapiro.test(raw1b$cbR_DbL)#worked

qqnorm(raw1b$cbR_BD)
qqline(raw1b$cbR_BD)

qqnorm(raw1b$cbR_PreDL)
qqline(raw1b$cbR_PreDL)

qqnorm(raw1b$cbR_DbL)
qqline(raw1b$cbR_PreDL)

#raw1b <- raw1b[-c(48:50)]
#need to cube root SL to use in regressions

raw1b$cbR_SL ='^'(raw1b$SL,1/3)
...
lat <- raw1b[raw1b$SPP == "p.latipinna",]


form <- raw1b[raw1b$SPP == "p.formosa",]


mex <- raw1b[raw1b$SPP == "p.mexicana",]
...
#let's get this into the raw1b data set so that we can plot this more easily
...
raw2 <- cbind(raw1b, abs.res.D, abs.res.P1, abs.res.P1.R, abs.res.LLSC, abs.res.SBLL, abs.res.BD, abs.res.CPD, abs.res.CPL, abs.res.PreDL, abs.res.DbL, abs.res.HL, abs.res.HD, abs.res.HW, abs.res.SnL, abs.res.OL)
...
ggplot(raw2, aes(SPP, abs.res.OL)) +
  ...
  stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), 
                 geom="pointrange", color="red")+
  expand_limits(y=c(min(raw2$abs.res.OL), max(raw2$abs.res.OL)))
...
raw1ba <- subset(raw1b, select = -c(17:18) ) #took out P2R and L since they don't vary at all
raw1ba <- subset(raw1ba, select=-c(34:49))#took out transformed values
raw1ba <- raw1ba[raw1ba$SPP !="p.mexicana", ]

#PCA <- prcomp(raw1ba[, 15:32], center=TRUE, scale. = TRUE) #includes all but pelvic traits, since they are the same in all species WILL SCALE/CENTER DATA ON MY OWN, LOGAN SAID IT IS NOT ALWAYS DOING IT FOR YOU EVEN WHEN YOU SPECIFY

z.scores <- scale(raw1ba[, 15:33], center = TRUE, scale = TRUE)

raw1ba <- cbind(raw1ba, z.scores)

colnames(raw1ba)[34:52] <- str_c( "z_", colnames(raw1ba)[15:33] )


PCA <- prcomp(raw1ba[, 34:52])
...
plot1<- autoplot(PCA, data = raw1ba, colour='SPP', loadings=FALSE, loadings.label=FALSE, frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot1
...
plot2<- autoplot(PCA, data = raw1ba, colour='QUARTILE', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot2
...
plot3<- autoplot(PCA, data = raw1ba, colour='BASIN', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot3
...
```{r}
plot4<- autoplot(PCA, data = raw1ba, colour='WATERSHED', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot4
...
plot5<- autoplot(PCA, x=2, y=3, data = raw1ba, colour='SPP', loadings=FALSE, loadings.label=FALSE, frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal()
plot5
...
plot6<- autoplot(PCA, x=2, y=3, data = raw1ba, colour='QUARTILE', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot6
...
plot7<- autoplot(PCA, x=2, y=3, data = raw1ba, colour='BASIN', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot7
...
plot8<- autoplot(PCA, x=2, y=3, data = raw1ba, colour='WATERSHED', shape="SPP", frame=TRUE, frame.type='norm')+ ggtitle("PCA Plot of Morphology traits") + theme_minimal() 
plot8
...
F.res.mex.P1 <- resid(F.reg.mex.P1)
...
rawFa <- raw1ba[rawFa$SPP !="p.mexicana", ]
