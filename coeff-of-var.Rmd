...
Not 100% sure which coeff will be most useful (or correct) for comparison, so for right now, I will do all combinations and remove what is not useful/correct after discussing with Mike. Combinations include:

  (1) Amazon vs Sailfin, each trait, raw data
  (2) Amazon vs sailfin, each trait, size corrected data
  (3) Amazon vs sailfin, PC1 data
  (4) Amazon vs sailfin, PC2 data
  (5) Amazon vs sailfin, PC3 data
  (6) Amazon across zones, each trait, raw data
  (7) Amazon across zones, each trait, size corrected data
  (8) Amazon across zones, PC1 data
  (9) Amazon across zones, PC2 data
  (10) Amazon across zones, PC3 data
  (11) Sailfin across zones, each trait, raw data
  (12) Sailfin across zones, each trait, size corrected data
  (13) Sailfin across zones, PC1 data
  (14) Sailfin across zones, PC2 data
  (15) Sailfin across zones, PC3 data
  (16) Amazon across basins, each trait, raw data
  (17) Amazon across basins, each trait, size corrected data
  (18) Amazon across basins, PC1 data
  (19) Amazon across basins, PC2 data
  (20) Amazon across basins, PC3 data
  (21) Sailfin across basins, each trait, raw data
  (22) Sailfin across basins, each trait, size corrected data
  (23) Sailfin across basins, PC1 data
  (24) Sailfin across basins, PC2 data
  (25) Sailfin across basins, PC3 data
  
There are only 4 of the 7 basins that are shared between the species, so I will only do those 4. Additionally, only 4 of the 10 watersheds are shared between the species, so don't think it's a worthwhile comparison to make. Will stick with zones and basins. 
  


## Combos 1-5

Amazon vs sailfin, each trait, raw, size corrected, and PCA data

***NOTE***: I'm not sure I can do all the traits by species for PCA data. I think I either get each individuals dimension for each PC axis (so would compare a single value for amz vs sailfin on PC1) OR I get the loadings for each trait, not separated by species. Going to go with the prior option, not sure if this is useful/correct.

```{r}
## separate species 
rawA <- raw1[raw1$SPP == "p.formosa",]
rawA <- rawA[, -c(17:18)]

rawS <- raw1[raw1$SPP == "p.latipinna",]
rawS <- rawS[, -c(17:18)]

##Amazon, raw data
amz_cv1 <- lapply(rawA[15:34], coef_var)

##Sailfin, raw data
lat_cv1 <- lapply(rawS[15:34], coef_var)

###### Not sure, but will do the transformed data as well

## Amazon, transformed (only metric traits)
amz_cv1a <- lapply(rawA[35:49], coef_var)

## Sailfin, transformed (only metric traits)
lat_cv1a <- lapply(rawS[35:49], coef_var)


######### Residuals (size corrected data)
## get residuals for each species into a dataframe

res_form <- data.frame(abs.form.D, abs.form.P1, abs.form.P1.R, abs.form.LLSC, abs.form.BD, abs.form.CPD, abs.form.CPL, abs.form.PreDL, abs.form.DbL, abs.form.HL, abs.form.HD, abs.form.HW, abs.form.SnL, abs.form.OL)

res_lat <- data.frame(abs.lat.D, abs.lat.P1, abs.lat.P1.R, abs.lat.LLSC, abs.lat.BD, abs.lat.CPD, abs.lat.CPL, abs.lat.PreDL, abs.lat.DbL, abs.lat.HL, abs.lat.HD, abs.lat.HW, abs.lat.SnL, abs.lat.OL)

## Amazon, size corrected (will not be all traits!)
amz_cv2 <- lapply(res_form, coef_var)


##sailfin, size corrected (will not be all traits!)
lat_cv2 <- lapply(res_lat, coef_var)


########## PCA data (see NOTE above)
##means are super small since there are neg/pos values, so not sure if this is useful

##Amazon, PC1
amz_cv3 <- coef_var(form.p$Dim.1)

##Sailfin, PC1
lat_cv3 <- coef_var(lat.p$Dim.1)

##Amazon, PC2
amz_cv4 <- coef_var(form.p$Dim.2)

##Sailfin, PC2
lat_cv4 <- coef_var(lat.p$Dim.2)

##Amazon, PC3
amz_cv5 <- coef_var(form.p$Dim.3)

##Sailfin, PC3
lat_cv5 <- coef_var(lat.p$Dim.3)
...
***NOTE***: See link below for full description. CV calculated on data on interval scales (ie can be pos or neg) can be inaccurate due to the mean being at/close to zero. Additionally, CV calculated on log-normal data is uninformative (stationary CV values?). Not sure if my transformed data meet this, as after log transforming they are more normal but not exactly normal, meaning the original data is only approximately log-normal. However, I calculate the CV on the transformed data, so might not matter.

https://cran.r-project.org/web/packages/cvequality/vignettes/how_to_test_CVs.html

```{r}
(raw_cv <- do.call(rbind, Map(data.frame, A=amz_cv1, S=lat_cv1)))

(trans_cv <- do.call(rbind, Map(data.frame, A=amz_cv1a, S=lat_cv1a)))

(size_cv <-do.call(rbind, Map(data.frame, A=amz_cv2, S=lat_cv2)))

(PCA_cv <- do.call(rbind, Map(data.frame, A.PC1=amz_cv3, S.PC1=lat_cv3, A.PC2=amz_cv4, S.PC2=lat_cv4, A.PC3=amz_cv5, S.PC3=lat_cv5)))
...
To see if these are statistically different, we can use the cvequality package on the data. This requires all the data I'm interested in testing to be in the same data frame (except PCA data, don't think it's useful to test on this data).

```{r}

#remove lat and form from column names so I can combine
names(res_lat) = gsub(pattern = ".lat.", replacement = "", x = names(res_lat))
names(res_form) = gsub(pattern = ".form.", replacement = "", x = names(res_form))

#add species column
res_lat$SPP <- 'p.latipinna'
res_form$SPP <- 'p.formosa'

size.corr <- rbind(res_lat, res_form)

#remove mexicana from raw data frame

raw.new <- raw1[raw1$SPP !="p.mexicana", ]

#add size corrected to data frame already with raw & transformed

raw.new <- cbind(raw.new, size.corr)

#also remove pelvic fin stuff

raw.new <- raw.new[, -c(17:18)]
...
***CANNOT FIGURE OUT HOW TO DO THIS EFFICIENTLY*** wILL TRY LATER


apply_asymptotic_test <- function(df, id_col, data_cols) {
   Extract the identifier column
  identifier_data <- df[[id_col]]

   Loop over the data columns and apply the test
  results <- lapply(data_cols, function(col_name) {
    column_data <- df[[col_name]]
    
    # Apply asymptotic_test
    test_result <- asymptotic_test(x = column_data, g = identifier_data)
    
    # Store results
    list(column = col_name, result = test_result)
  })

  return(results)
}

 Specify the identifier column and columns to test
identifier_column <- "SPP"              Species ID column
columns_to_test <- c("D", "P1", "A", "P1.R", "LLSC", "SALL", "SBLL", "SBDF", "SL", "BD", "CPD", "CPL", "PreDL", "DbL", "HL", "HD", "HW", "SnL", "OL", "FLA")

 Apply the function
results <- apply_asymptotic_test(df, identifier_column, columns_to_test)

 Print results
for (res in results) {
  cat("Column:", res$column, "\n")
  print(res$result)
  cat("\n")
}
...
with (raw.new, asymptotic_test(D, SPP))
with (raw.new, asymptotic_test(P1, SPP))
with (raw.new, asymptotic_test(A, SPP))
with (raw.new, asymptotic_test(P1.R, SPP))
with (raw.new, asymptotic_test(LLSC, SPP))
with (raw.new, asymptotic_test(SALL, SPP))
with (raw.new, asymptotic_test(SBLL, SPP))
with (raw.new, asymptotic_test(SBDF, SPP))
with (raw.new, asymptotic_test(SL, SPP))
with (raw.new, asymptotic_test(BD, SPP))
with (raw.new, asymptotic_test(CPD, SPP))
with (raw.new, asymptotic_test(CPL, SPP))
with (raw.new, asymptotic_test(PreDL, SPP))
with (raw.new, asymptotic_test(DbL, SPP))
with (raw.new, asymptotic_test(HL, SPP))
with (raw.new, asymptotic_test(HD, SPP))
with (raw.new, asymptotic_test(HW, SPP))
with (raw.new, asymptotic_test(SnL, SPP))
with (raw.new, asymptotic_test(OL, SPP))
with (raw.new, asymptotic_test(FLA, SPP))

### AvS, transformed data
with (raw.new, asymptotic_test(SL_log, SPP))
with (raw.new, asymptotic_test(BD_log, SPP))
with (raw.new, asymptotic_test(CPD_log, SPP))
with (raw.new, asymptotic_test(CPL_log, SPP))
with (raw.new, asymptotic_test(cbR_PreDL, SPP))
with (raw.new, asymptotic_test(cbR_DbL, SPP))
with (raw.new, asymptotic_test(HL_log, SPP))
with (raw.new, asymptotic_test(HD_log, SPP))
with (raw.new, asymptotic_test(HW_log, SPP))
with (raw.new, asymptotic_test(SnL_log, SPP))
with (raw.new, asymptotic_test(OL_log, SPP))


### AvS, size corrected data
with (raw.new, asymptotic_test(absD, SPP))
with (raw.new, asymptotic_test(absP1, SPP))
with (raw.new, asymptotic_test(absP1.R, SPP))
with (raw.new, asymptotic_test(absLLSC, SPP))
with (raw.new, asymptotic_test(absBD, SPP))
with (raw.new, asymptotic_test(absCPD, SPP))
with (raw.new, asymptotic_test(absCPL, SPP))
with (raw.new, asymptotic_test(absPreDL, SPP))
with (raw.new, asymptotic_test(absDbL, SPP))
with (raw.new, asymptotic_test(absHL, SPP))
with (raw.new, asymptotic_test(absHD, SPP))
with (raw.new, asymptotic_test(absHW, SPP))
with (raw.new, asymptotic_test(absSnL, SPP))
with (raw.new, asymptotic_test(absOL, SPP))
...
From the raw data: D, LLSC, and CPL were sig, SBLL and PreDL suggestive

From the transformed data: SL_log and OL_log were sig, HW_log suggestive

From the size corrected data: absP1 and absP1.R were sig